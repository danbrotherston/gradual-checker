...
public class ReplacingTreeTranslator extends HelpfulTreeTranslator<GradualNullnessChecker> {
	...
	replacementMap = new HashMap<JCTree, JCTree>();
	...
	for (Map.Entry<TreePath, Map.Entry<Tree, AnnotatedTypeMirror>> location :
	    ...
	    replacementMap.put((JCTree) (statementToReplace.getLeaf()),
	...
	if (replacementMap.containsKey(that)) {
		...
		result instanceof JCTree.JCExpression) {
		this.attribute((JCTree.JCExpression) that,
			       (JCTree.JCExpression) result);
	    } else if (that instanceof JCTree.JCStatement &&
		       result instanceof JCTree.JCStatement) {
		this.attribute((JCTree.JCStatement) that,
